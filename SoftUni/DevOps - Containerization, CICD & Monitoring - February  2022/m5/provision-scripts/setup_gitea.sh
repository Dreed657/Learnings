...
# Wait until ready
while true; do 
  echo 'Trying to connect to http://192.168.56.102:3000 ...'; 
  if [ $(curl -s -o /dev/null -w "%{http_code}" http://192.168.56.102:3000) == "200" ]; then 
    echo '... connected.'; 
    break; 
  else 
...
cd /tmp/bgapp && git push -o repo.private=false http://vagrant:vagrant@192.168.56.102:3000/vagrant/bgapp

# Add a Webhook
curl -X 'POST' 'http://192.168.56.102:3000/api/v1/repos/vagrant/bgapp/hooks' \
  ...
  "config": {
    "content_type": "json",
    "url": "http://192.168.56.101:8080/gitea-webhook/post",
    "http_method": "post"
  },
  "events": [
    "push"
